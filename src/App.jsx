    ...
    questions: [
      {
        ...
        answer: 1,
        explanation:
          "Titik ada di dalam contour jika nilai (X-μ)'Σ⁻¹(X-μ) tidak melebihi batas kritis chi-square untuk contour tersebut.",
      },
      {
        id: "C2",
        points: 110,
        ...
        answer: 1,
        explanation:
          "Eigenvector menentukan arah sumbu ellips, sedangkan eigenvalue mengatur panjangnya.",
      },
      {
        id: "C3",
        points: 120,
        ...
        answer: 2,
        explanation:
          "Contour 95% mencakup area peluang lebih besar, sehingga ellips terlihat lebih besar.",
      },
    ],
  },
  {
    ...
    questions: [
      {
        ...
        answer: 1,
        explanation:
          "Bila data mendekati normal multivariat, ordered distance dan quantile teoritis cenderung mengikuti garis lurus.",
      },
      {
        id: "O2",
        points: 150,
        ...
        answer: 2,
        explanation:
          "Mahalanobis distance memperhitungkan kovarians, sehingga cocok untuk deteksi outlier multivariat.",
      },
      {
        id: "O3",
        points: 160,
        ...
        answer: 1,
        explanation:
          "Penyimpangan besar dari garis acuan biasanya menunjukkan outlier atau distribusi yang tidak normal multivariat.",
      },
    ],
  },
  {
    ...
    questions: [
      {
        ...
        answer: 1,
        explanation:
          "Dalam keluarga Box-Cox, λ = 0 bersesuaian dengan transformasi logaritma natural.",
      },
      {
        id: "B2",
        points: 190,
        ...
        answer: 1,
        explanation:
          "Transformasi Box-Cox dipakai untuk membantu memenuhi asumsi normalitas dengan membuat distribusi lebih simetris.",
      },
      {
        id: "B3",
        points: 200,
        ...
        answer: 1,
        explanation:
          "Jika λ mendekati 1, bentuk asli data sudah cukup baik sehingga transformasi besar biasanya tidak diperlukan.",
      },
    ],
  },
  {
    ...
    questions: [
      ...
      },
      {
        ...
        answer: 1,
        explanation:
          "Dalam distribusi normal ganda, kovarians nol mengimplikasikan independensi.",
      },
      {
        id: "Q3",
        points: 140,
        ...
        answer: 1,
        explanation:
          "Pelanggaran normalitas marginal pada satu atau lebih variabel adalah sinyal kuat bahwa normalitas multivariat meragukan.",
      },
    ],
  },
];
...
export default function MVNBattleArenaRealtimeDB() {
  ...
  useEffect(() => {
    try {
      ...
      setDb(database);
      setNotice("Realtime Database terkoneksi.");
    } catch (err) {
  ...
  const renderLeaderboard = (roundOnly = false) => {
    ...
    const list =
      roundOnly && roundKey
        ? [...teams].sort(
            (a, b) =>
              (b.roundScores?.[roundKey] || 0) - (a.roundScores?.[roundKey] || 0) ||
              (b.score || 0) - (a.score || 0)
          )
        : sortedTeams;
              ...
              <tr key={team.id} className="border-t border-white/10 hover:bg-white/[0.03]">
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center gap-3">
                    <div
                      className="flex h-10 w-10 items-center justify-center rounded-2xl text-xs font-black text-slate-950 shadow-lg"
                ...
                <td className="px-4 py-3 font-bold text-white">
                  {roundOnly ? team.roundScores?.[roundKey] || 0 : team.score || 0}
                </td>
  ...
  const currentRoundProgress =
    room && currentRound
      ? ((room.currentQuestionIndex +
          (room.phase === "reveal" || room.phase === "roundSummary" || room.phase === "final" ? 1 : 0)) /
  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(circle_at_top_right,rgba(56,189,248,0.14),transparent_26%),radial-gradient(circle_at_top_left,rgba(168,85,247,0.14),transparent_24%),linear-gradient(180deg,#020617_0%,#020617_28%,#0f172a_100%)] text-slate-100">
      <div className="mx-auto max-w-7xl px-4 py-6 lg:px-6">
          ...
          <div className="relative z-10 flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
            <div className="max-w-3xl">
              ...
              <p className="mt-3 text-base leading-7 text-slate-300 sm:text-lg">
                Game interaktif multivariat dengan mode <strong>host + room + device per tim</strong>.
              </p>
              <div className="mt-5 flex flex-wrap gap-3 text-sm text-slate-300">
                <Tag icon={Database} text="Auto-connect RTDB" />
            ...
            <div className="grid w-full max-w-md grid-cols-2 gap-3 lg:w-[360px]">
              ...
              <MetricCard
                ...
                icon={db ? CheckCircle2 : HelpCircle}
              />
            </div>
          </div>
        </div>
        ...
        {connectError ? (
          <div className="mt-6 rounded-3xl border border-rose-400/20 bg-rose-400/10 p-4 text-rose-100">
            {connectError}
          </div>
        ) : null}

        {notice ? (
          <div className="mt-6 rounded-3xl border border-emerald-400/20 bg-emerald-400/10 p-4 text-emerald-100">
            {notice}
          </div>
        ) : null}
        ...
        <div className="mt-6">
          <GlassCard>
            <SectionTitle
              icon={LayoutGrid}
              title="Masuk ke Room"
              subtitle="Pilih mode host atau tim. Koneksi database berjalan otomatis saat halaman dibuka."
            />
            <div className="mb-5 inline-flex rounded-2xl border border-white/10 bg-white/5 p-1">
              <button
                onClick={() => setTab("host")}
                className={`inline-flex items-center gap-2 rounded-xl px-4 py-2 text-sm font-semibold ${
                  tab === "host" ? "bg-cyan-500 text-slate-950" : "text-slate-200"
                }`}
              >
              ...
              <button
                onClick={() => setTab("team")}
                className={`inline-flex items-center gap-2 rounded-xl px-4 py-2 text-sm font-semibold ${
                  tab === "team" ? "bg-cyan-500 text-slate-950" : "text-slate-200"
                }`}
              >
            ...
            {tab === "host" ? (
              <div className="space-y-4">
                ...
                <InfoPanel icon={PlusCircle} title="Buat room baru">
                  Room memakai limit <strong>{MAX_TEAMS} tim</strong>.
                </InfoPanel>
                <button
                  disabled={!db}
                  onClick={createRoom}
                  className="inline-flex items-center gap-2 rounded-2xl bg-indigo-500 px-4 py-3 font-semibold text-white transition hover:bg-indigo-400 disabled:cursor-not-allowed disabled:opacity-50"
                >
              ...
              <div className="space-y-4">
                    ...
                    {TEAM_COLORS.map((c) => (
                      <button
                        ...
                        className={`h-11 w-11 rounded-2xl border-2 transition ${
                          teamColor === c ? "border-white scale-105" : "border-transparent"
                        }`}
                        style={{ background: c }}
                      />
                    ))}
                ...
                <button
                  disabled={!db}
                  onClick={joinRoom}
                  className="inline-flex items-center gap-2 rounded-2xl bg-emerald-500 px-4 py-3 font-semibold text-slate-950 transition hover:bg-emerald-400 disabled:cursor-not-allowed disabled:opacity-50"
                >
                ...
                <div className="space-y-3">
                  {teams.length === 0 ? (
                    <EmptyState
                      icon={Users}
                      title="Belum ada tim"
                      desc="Minta tiap tim join menggunakan kode room yang sudah dibagikan host."
                    />
                  ) : (
                    sortedTeams.map((team, idx) => (
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <div
                            className="flex h-12 w-12 items-center justify-center rounded-2xl text-sm font-black text-slate-950 shadow-lg"
                            style={{ background: team.color }}
                          >
                            {initials(team.name)}
                          </div>
              ...
              <div className="mb-5 flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                <div>
                  ...
                  <h2 className="text-2xl font-black tracking-tight sm:text-3xl">
                    {currentRound?.title || "Menunggu"}
                  </h2>
                  <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-400 sm:text-base">
                    {currentRound?.description || "Menunggu room aktif."}
                  </p>
                </div>
                <div className="flex flex-wrap items-center gap-3">
                  {currentQuestion ? (
                    <StatPill
                      icon={TimerReset}
                      label={`Timer ${room.phase === "question" ? `${timeLeft}s` : `${currentQuestion.timeLimit}s`}`}
                      tone="amber"
                    />
                  ) : null}
                  ...
                  {identity ? (
                    <StatPill
                      icon={identity.role === "host" ? Crown : Smartphone}
                      label={identity.role === "host" ? "Mode Host" : `Mode Tim${myTeam ? ` • ${myTeam.name}` : ""}`}
                      tone="slate"
                    />
                  ) : null}
                </div>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="mt-1 text-sm text-slate-400">
                          Semua tim bisa tetap di layar masing-masing sambil menunggu host memulai game.
                        </p>
                      </div>
                    </div>
                    {isHost ? (
                      <div className="mt-5 flex flex-wrap gap-3">
                        <button
                          onClick={startGame}
                          className="inline-flex items-center gap-2 rounded-2xl bg-cyan-500 px-4 py-3 font-semibold text-slate-950 transition hover:bg-cyan-400"
                        >
                          <Play className="h-4 w-4" /> Start Game
                        </button>
                        <button
                          onClick={resetRoom}
                          className="inline-flex items-center gap-2 rounded-2xl bg-white/10 px-4 py-3 font-semibold text-slate-200 transition hover:bg-white/15"
                        >
              ...
              {room.phase === "question" && currentQuestion ? (
                <div className="grid gap-6 lg:grid-cols-[1.08fr_0.92fr]">
                  ...
                  >
                    ...
                    <GlassSubCard
                      className={`bg-gradient-to-br ${currentRound?.accent || "from-cyan-400/10 to-slate-950"}`}
                    >
                            ...
                            return (
                              <motion.button
                                ...
                                className={`group rounded-3xl border px-4 py-4 text-left transition ${
                                  selected
                                    ? "border-cyan-400 bg-cyan-400/10 text-cyan-100 shadow-[0_0_0_1px_rgba(34,211,238,0.15)]"
                                  ...
                                  <div className="flex-1">
                                    ...
                                    {selected ? (
                                      <div className="mt-1 text-xs text-cyan-200">
                                        Jawaban timmu terkirim
                                      </div>
                                    ) : null}
                                    {locked && !selected ? (
                                      <div className="mt-1 text-xs text-slate-400">
                                        Pilihan lain otomatis terkunci
                                      </div>
                                    ) : null}
                                  </div>
                                </div>
                    ...
                    <GlassSubCard>
                      <div className="mb-4 flex items-center justify-between gap-3">
                        <div>
                          ...
                          <p className="mt-1 text-sm text-slate-400">
                            Reveal answer akan menghitung skor semua tim secara realtime.
                          </p>
                        </div>
                      ...
                      {isHost ? (
                        <div className="mt-4 flex flex-wrap gap-3">
                          <button
                            ...
                            className="inline-flex items-center gap-2 rounded-2xl bg-indigo-500 px-4 py-3 font-semibold text-white transition hover:bg-indigo-400"
                          >
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p className="mt-2 text-slate-300">
                          {currentRound?.title} selesai. Cek skor round ini dan total akumulasi semua round.
                        </p>
                      </div>
                    </div>
                    {isHost ? (
                      ...
                      <div className="mt-5 text-sm text-slate-400">
                        Menunggu host memulai round berikutnya.
                      </div>
                    )}
                  </GlassSubCard>
                  <div className="space-y-6">
...
function GlassCard({ children, className = "" }) {
  return (
    <div
      className={`rounded-[32px] border border-white/10 bg-slate-900/80 p-6 shadow-[0_30px_80px_rgba(0,0,0,0.35)] ${className}`}
    >
      {children}
    </div>
  );
  ...
  return (
    <div className="rounded-3xl border border-white/10 bg-white/5 p-4">
      <div className="flex items-center gap-2 text-slate-400">
        <Icon className="h-4 w-4" /> <span className="text-xs uppercase tracking-[0.16em]">{label}</span>
      </div>
...
function RoundVisualPanel({ round, question, questionIndex, totalQuestions, timeLeft, answersCount, totalTeams }) {
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.35 }}
      className={`relative overflow-hidden rounded-[28px] border border-white/10 bg-gradient-to-br ${
        round?.accent || "from-cyan-400/10 to-slate-950"
      } p-6`}
    >
...
function EmptyState({ icon: Icon, title, desc }) {
  return (
    <div className="rounded-3xl border border-dashed border-white/10 bg-white/[0.03] p-8 text-center">
      <div className="mx-auto mb-3 inline-flex rounded-2xl border border-white/10 bg-slate-950/50 p-3 text-slate-300">
